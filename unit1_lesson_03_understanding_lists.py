...
def test_list_can_be_sliced():
    ...
    #begin : end
    assert ['banana', 'orange'] == fruits[0:2],"01"
    assert ['banana', 'orange', 'grape'] == fruits[0:5],"02"
    assert ['orange'] == fruits[1:-1],"03"

    # begin :
    assert ['banana', 'orange', 'grape'] == fruits[0:],"11"
    assert ['grape'] == fruits[2:],"12"
    assert ['banana', 'orange', 'grape'] == fruits[0:],"13"

    #: end
    assert [] == fruits[:0],"21"
    assert ['banana', 'orange'] == fruits[:2],"22"
    assert ['banana', 'orange', 'grape'] == fruits[:5],"23"

    # note the invariant
    assert ['banana', 'orange', 'grape'] == fruits[:1] + fruits[1:],"31"
...
def test_list_merge():
    ...
    assert ['banana', 'orange', 'grape', 'beetroot', 'tomato'] == all,"1"
    assert ['banana', 'orange', 'grape'] == fruits,"2"
    assert ['beetroot', 'tomato'] == veggies,"3"
    assert ['orange', 'grape', 'beetroot'] == fruits[1:] + veggies[:1],"4"
...
def test_list_slice_replacement_is_inplace():
    ...
    fruits[3:] = []
    assert ['banana', 'litchi', 'guava'] == fruits,"2"
...
def test_list_common_methods():
    ...
    assert ['orange'] == fruits,"3"

    fruits.insert(0, "banana")
    assert ['banana','orange'] == fruits,"4"

    fruits.extend(["litchi", "guava"])
    assert ['banana','orange', 'litchi', 'guava'] == fruits,"5"

    fruits.reverse()
    assert ['guava', 'litchi', 'orange', 'banana'] == fruits,"1"

    fruits.pop()
    assert ['guava', 'litchi', 'orange'] == fruits,"2"

    fruits.pop(0)
    assert ['litchi', 'orange'] == fruits,"9"
...
-range can be incereemented if we put a +ve no. in step and decreement if a -ve no. is is step.
-if a string is passed to the list() function, then each character can be broken into elements of a list.
-'in' has more preceedence than '=='
"""