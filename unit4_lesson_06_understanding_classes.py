...
def test_classes_are_object_factories():
    ...
    x=q1.__class__
    y=q2.__class__
    assert x == q1.__class__
    assert y == q2.__class__
...
def test_classes_int_with_args():
    class TestQueue:
...
def test_classes_bound_and_unbound_methods():
    ...
    x=TestQueue.push
    assert x == TestQueue.push   #unbound method
    y=q1_push.__self__
    assert y == q1_push.__self__  #bound method, is associated with an instance
