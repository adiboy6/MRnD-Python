...
def min_length(seq1, seq2, *more_seqs):
    ...
    if more_seqs:
        print("seq1=",len(seq1),"seq2=", len(seq2),"more_seqs=",len(more_seqs))
        return min(len(seq1), len(seq2), len(min(more_seqs)))
    else:
...
def buggy_contains(current, new):
    if ((new[:-6].lower()+new[-6:]) in current or (new[:-6].upper()+new[-6:]) in current):
        return True
    return False
