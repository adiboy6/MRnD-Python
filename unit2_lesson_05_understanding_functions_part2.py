...
def test_function_call_with_keyword_arguments():
    assert [10,16,3,4] == demo(10,16)
    assert [10,20,3,4] == demo(10, 20)
    assert [10,20,30,4] == demo(10, 20, 30)
    assert [10,20,3,4] == demo(first=10,second=20)
    assert [10,20,30,4] == demo(10,second=20, third=30)
    assert [10,2,30,4] == demo(first=10, second=2, third=30)
    assert [10,2,30,4] == demo(10,2, third=30)
    assert [1,20,3,10] == demo(1,second=20, fourth=10)  # is this allowed? correct and uncomment"""it's not allowed because whenever we use a keyword argument, then after that we must compulsory use positional argument it no matter what.
...
def test_function_with_keyword_args():
    ...
    assert {} == demo_with_keyword_args(name="jack") # what do you observe here?Observation:Keyword arguments should have atleast 2 arguments in it
...
-a keyword argument if used should be followed by another keyword argument
-*args represents tuple and **kwargs represents dict and the arguments are based upon FCFS basis.
-
"""
