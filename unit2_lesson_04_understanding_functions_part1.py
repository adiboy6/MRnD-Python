...
def get_word_with_least_vowels(input):
    ...
    """
    return min(list(input),key=checkvowel) # replace this with right code. Use min builtin and define a new function to pass as key.
...
def get_min_max_words(input):
    ...
    """
    return (min(list(input),key=len),max(list(input),key=len))
...
-When there's nothing t return, the function returns None object and NoneType is not callable
-when a number is passed, then its data cannot be retained after any changes but whereas when a list is passed, then it retains its value when function ends.
-
"""