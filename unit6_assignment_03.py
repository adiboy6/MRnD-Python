...
def anagram_sort(source, destination):
    ...
    k=sorted(sorted([x for x in anagrams.keys() if len(anagrams[x])>1],key=len,reverse=True))
    s=""
    for i in sorted(sorted([x for x in anagrams.keys() if len(anagrams[x])>1],key=len,reverse=True)):
        s+="\n".join(anagrams[i])+'\n'
    for i in sorted(sorted([x for x in anagrams.keys() if len(anagrams[x])==1],key=len,reverse=True)):
        s+="\n".join(anagrams[i])+'\n'
