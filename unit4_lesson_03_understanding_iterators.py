...
def test_enumerate_iter():
    ...
    try:
        ...
        assert [] == list_iter.__next__() #note what happens when items are finished.
...
def test_find_builtins_that_work_on_iterables():
    funcs = [all,any,enumerate,list,map,tuple,max,min,set,sorted,sum,zip]
    assert 12 == len(funcs)
...
def test_my_map():
    ...
    assert list(string.ascii_uppercase) == my_map(upper, string.ascii_lowercase)
