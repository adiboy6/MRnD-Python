...
def are_anagrams(first, second):
    if(first==None or second==None):
        return None
    return set(first.lower())==set(second.lower())
...
def test_are_anagrams_student():
    ...
    assert True == are_anagrams("pit", "tpi")
    assert True == are_anagrams("edward daniles", "andrew laeddis")
    assert True == are_anagrams("dolores chanal", "rachel solando")
    assert False == are_anagrams("pi t", "tip")
