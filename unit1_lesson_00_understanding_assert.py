...
the <optional message>. If it evaluates to True, nothing happens and execution moves to the next statement in the function.
...
def test_assert_true_with_message():
    print("test print")
    assert 1, "This is the failure message" # what error do you see when you run this test?
...
def test_make_assert_true_1():
    assert 7+1 > 7, "Fill in a value greater than 7"
...
-Value in ("") or ('') are same but what I found was that double quotes are strings which are visible for user and single single quotes are for strings which relate to the funstionallity of the code.
-** represents power and power can be any expression written in paranthesus.
-assertion is just like an if where it raises an alarm if there's an exception. It accepts only True value in it.
-"___" represents Flase or 0 which is defined in placeholder
"""