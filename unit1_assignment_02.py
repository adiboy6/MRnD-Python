...
def get_encoding_dict():
    ...
    """
    return dict(zip(string.ascii_lowercase+string.ascii_uppercase,list(range(1,27))+list(range(1,27))))
...
def get_consonents(input):
    ...
    No control flow plz.
    """
    return set(input.lower())-set(VOWEL_SET)
...
def capitalize_names(input):
    ...
    """
    return list(map(str.capitalize,input))
...
def bit_count_even(number):
    ...
    """
    return not bool(bin(number).count('1') % 2)
