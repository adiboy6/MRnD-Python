...
def test_dictionary_keys_and_values():
    ...
    keys = sorted(word_to_digit.keys())
    assert ['one','two'] == keys
    values = word_to_digit.values()
    values = sorted(word_to_digit.values())
    assert [1,2] == values
...
def make_dict(keys, values):
    ...
    """
    return dict(zip(list(keys),list(values))) # write a single line of code to create the dict.
...
-zip is used to cluster two list to form a tuple and then to form a dictionary
-key value shouldn't be null
-the dictionary is in the ascending order of values
"""