...
def generator_zip(seq1, seq2, *more_seqs):
    ...
    """
    if more_seqs==():
        if (type(seq1).__name__== 'str' or type(seq1).__name__== 'list' or type(seq1).__name__== 'tuple' or type(seq1).__name__== 'range') and ( type(seq2).__name__== 'range' or type(seq2).__name__== 'str' or type(seq2).__name__== 'list' or type(seq2).__name__== 'tuple'):
            t=min(len(seq1),len(seq2))
            for x in range(t):
                z = (seq1[x], seq2[x])
                yield z
        else:
            if type(seq1).__name__ == 'str' or type(seq1).__name__ == 'list' or type(seq1).__name__ == 'tuple' or type(seq1).__name__ == 'range':
                t = min(len(seq1))
                l=seq1
                m=seq2
            else:
        ...
        if (type(seq1).__name__== 'str' or type(seq1).__name__== 'list' or type(seq1).__name__== 'tuple' or type(seq1).__name__== 'range') and ( type(seq2).__name__== 'range' or type(seq2).__name__== 'str' or type(seq2).__name__== 'list' or type(seq2).__name__== 'tuple'):
            ...
            if type(seq1).__name__ == 'str' or type(seq1).__name__ == 'list' or type(seq1).__name__ == 'tuple' or type(seq1).__name__ == 'range':
                t = min(len(seq1), min([len(x) for x in more_seqs]))
                l=seq1
                m=seq2
            else:
                t = min(len(seq2), min([len(x) for x in more_seqs]))
                l=seq2
                m=seq1
            for x in range(t):
                z = (l[x], next(m)) + tuple([more_seqs[i][x] for i in range(len(more_seqs))])
                yield z
