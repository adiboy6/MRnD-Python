...
def test_string_concat():
    assert 'Hello  world' == "Hello " + " world","1"
    assert 'Hello world' == """Hello """ + 'world',"2"
    assert 'Hello world' == 'Hello ' + "world","3"
...
def test_string_slicing():
    ...
    string = "Hello world"
    #with begin : end
    assert ''== string[0:0],"01"

    assert 'He'== string[0:2],"02"
    assert 'ello'== string[1:5],"03"
    assert 'ello worl'== string[1:-1]
    assert 'llo wor'== string[2:-2]

    #with :end, note usage of negative indices as well.
    assert ''== string[:0],"1"
    assert 'Hell'== string[:4],"2"
    assert 'Hello worl'== string[:-1],"3"

    #with begin:
    assert 'Hello world'== string[0:],"11"
    assert 'o world'== string[4:],"22"
    assert 'd'== string[-1:],"33"

    #observe the invariant
    assert 'Hello world'  == string[:0] + string[0:],"11"
    assert 'Hello world' == string[:1] + string[1:],"12"
    assert 'Hello world' == string[:2] + string[2:],"13"
    assert 'Hello world' == string[:3] + string[3:],"14"
...
def test_string_membership():
    assert False == ('c' in 'apple'),"1"  #is there a precedence issue here or something else?
    assert True == ('a' in 'apple'),"2"
    assert True == ('app' in 'apple'),"3"
...
-while using "slicing", we start from "left" and just below "right" [i.e,string[left:right]
-a defined value can be increased within {}.
-strings cannot be pdated unlike C/C++
"""
